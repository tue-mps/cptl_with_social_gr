...
from data.loader_interaction import data_loader_interaction, data_dset_interaction
...
def train_cl(args, best_ade, model, train_datasets, val_datasets, replay_model="none", iters=2, batch_size=32,
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        if args.val:
            # if Generative:
            #     replay_data_loader_val = iter(data_loader(args, val_datasets[task-2], 256))
            #     replay_out_val = next(replay_data_loader_val)
            #     replay_traj_val = previous_generator.sample(replay_out_val[2].to(device), replay_out_val[0].to(device),
            #                                             replay_out_val[6].to(device))
            #     x_rel_val_ = replay_traj_val[1]
            #     seq_start_end_val = replay_traj_val[2]
            #     previous_model.eval()
            #     y_rel_val = previous_model(x_rel_val_, seq_start_end_val)
            #     x_rel_val = x_rel_val_
            # else:
            x_rel_val = None
            y_rel_val = None
        ...
        for epoch in range(1, iters_to_use+1):
            ...
            for batch_index, batch in enumerate(training_dataset):
                ...
                if Generative:
                    ...
                    replay_data_loader = iter(data_loader_interaction(args, train_dataset, args.replay_batch_size))
                    # replay_data_loader = iter(data_loader(args, train_dataset, args.replay_batch_size)) # replay_batch_size=512
                    ...
                    if args.replay_model == 'lstm':
                        ...
                        # fake seq_start_end
                        # obs_traj_rel[0], obs_traj[0], seq_start_end
                        # obs_traj_rel:[8,traj,2]
                        seq_ped_num = np.random.randint(1, high=10, size=args.replay_batch_size)
                        cum_start_idx = [0] + np.cumsum(seq_ped_num).tolist()
                        seq_start_end_fake = [(start, end) for start, end in zip(cum_start_idx, cum_start_idx[1:])]
                        num_traj = cum_start_idx[-1]
                        obs_traj_rel_fake = np.zeros((num_traj, 2))
                        shape = (num_traj, 2)
                        obs_traj_fake = np.array([10 * np.random.random_sample(shape[1]) - 10 for i in range(shape[0])])

                        seq_start_end_fake_ = torch.LongTensor(seq_start_end_fake)
                        obs_traj_rel_ = torch.from_numpy(obs_traj_rel_fake).type(torch.float)
                        obs_traj_rel_ = torch.unsqueeze(obs_traj_rel_, 0)
                        obs_traj_ = torch.from_numpy(obs_traj_fake).type(torch.float)
                        obs_traj_ = torch.unsqueeze(obs_traj_, 0)
                        replay_traj = previous_generator.sample(obs_traj_rel_.to(device), obs_traj_.to(device),
                                                                seq_start_end_fake_.to(device))
