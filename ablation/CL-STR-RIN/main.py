...
eval_params.add_argument('--loss-log', type=int, default=40, metavar="N", help="iters after which to plot loss")
eval_params.add_argument('--prec-log', type=int, default=40, metavar="N", help="iters after which to plot precision")
...
def run(args, verbose=False):
    ...
    if args.method == "continual_learning":
        ...
        train_datasets = []
        train_datasets_interaction = []
        val_datasets = []
        val_datasets_interaction = []
        val_dataset = []
        test_datasets = []
        ...
        # -visdom-settings
        if args.visdom:
            env_name = "epoch-social-GAN-fix-replay-{exp}-{tasks}-{iters}-{z_dim}-{batch_size}-{replay_batch_size}-{lr}-{seed}-{val}-{val_class}-si{si}-{si_c}".format(exp=args.dataset_order, tasks=tasks, iters=args.iters, z_dim=args.z_dim, batch_size=args.batch_size, replay_batch_size=args.replay_batch_size, lr=args.lr, seed=args.seed, val=args.val, val_class=args.val_class, si=args.si, si_c=args.si_c)
