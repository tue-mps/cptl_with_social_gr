...
class AutoEncoder(Replayer):
    ...
    def decode(self, z, seq_start_end, obs_traj_pos=None):
        ...
        len = self.obs_len + self.pred_len
    ...
    def forward(self, obs_traj_pos, pred_traj_pos, seq_start_end):
        ...
        input = torch.cat((obs_traj_pos, pred_traj_pos), dim=0)
    ...
    def train_a_batch(self, x_rel, y_rel, seq_start_end, x_=None, y_=None, seq_start_end_=None, rnt=0.5):
        ...
        if x_rel is not None:
            ...
            # Run the model
            recon_batch, mu, logvar, z = self(x_rel, y_rel, seq_start_end)
            ...
            traj_gt = torch.cat((x_rel, y_rel), dim=0)
            reconL, variatL = self.loss_function(recon_x=recon_batch, x=traj_gt, y_hat=None, y_target=None, mu=mu, logvar=logvar)
        ...
        if x_ is not None:
            ...
            if (not type(x_)==list):
                x_temp_ = x_
                recon_batch, mu, logvar, z = self(x_temp_, y_, seq_start_end_)
            ...
            for replay_id in range(n_replays):
                ...
                # Calculate all losses
                traj_gt_ = torch.cat((x_temp_, y_), dim=0)
                reconL_r[replay_id], variatL_r[replay_id] = self.loss_function(
                    recon_x=recon_batch, x=traj_gt_, mu=mu, logvar=logvar
                )
